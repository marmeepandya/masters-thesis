...
\begin{table}[ht]
\centering
\caption{SimLM results (relaxed top-1\,000 protocol) alongside the
         strongest baselines evaluated so far, for reference.}
\label{tab:simlm}
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{NDCG@10} & \textbf{Recall@1000} \\
\midrule
BM25 (lexical baseline, Section~\ref{sec:baseline_sparse}) & 0.854 & 0.635 \\
SimLM                                                       & 0.923 & 0.609 \\
MiniLM (Section~\ref{subsec:minilm})                        & 0.975 & 0.741 \\
GTE-large (Section~\ref{sec:extended_baselines})            & 0.946 & 0.768 \\
\bottomrule
\end{tabular}
\end{table}
...
despite SimLM being purpose-pretrained for passage retrieval. A likely
...
\begin{table}[ht]
\centering
\caption{ColBERT (brute-force MaxSim) results (relaxed top-1\,000
...
\begin{table}[ht]
\centering
\caption{Full-pool reranking results (relaxed top-1\,000 protocol),
         compared against the shallow RRF~+~top-50-rerank baseline from
         Section~\ref{sec:rrf}. Bold indicates the best value in each
         column.}
\label{tab:hybrid_fullpool}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{NDCG@10} & \textbf{Recall@1000} & \textbf{Avg.\ pool size} \\
\midrule
RRF~+~top-50 rerank (Section~\ref{sec:rrf})    & 0.979 & 0.782 & 50 (fixed) \\
Full-pool, 2-way (MiniLM~+~Linq)               & 0.955 & 0.802 & $\approx$1\,300 \\
Full-pool, 3-way (+~GTE-large)                 & 0.955 & \textbf{0.804} & $\approx$1\,424 \\
Full-pool, 4-way (+~OpenAI-large)              & \textbf{0.955} & 0.802 & $\approx$1\,600 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Reranker ablation: identical 3-way candidate pool, two different
         reranker models (relaxed top-1\,000 protocol).}
\label{tab:reranker_ablation}
\begin{tabular}{lcc}
\toprule
\textbf{Reranker} & \textbf{NDCG@10} & \textbf{Recall@1000} \\
\midrule
BAAI/bge-reranker-v2-m3 (Section~\ref{sec:hybrid_fullpool})  & 0.955 & 0.8042 \\
mixedbread-ai/mxbai-rerank-large-v1                          & \textbf{0.971} & 0.8035 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Learned fusion ranker results (held-out, cross-validated
...
\textbf{Method} & \textbf{NDCG@10} & \textbf{Recall@1000} \\
\midrule
Full-pool, 3-way + BGE reranker (Section~\ref{sec:hybrid_fullpool}) & 0.955 & 0.804 \\
Learned fusion, logistic regression                                 & 0.973 & 0.821 \\
Learned fusion, gradient-boosted trees                               & \textbf{0.979} & \textbf{0.823} \\
\bottomrule
\end{tabular}
\end{table}
