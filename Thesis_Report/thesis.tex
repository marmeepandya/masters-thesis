...
\paragraph{Status.} The combined pool and nested tier membership have been constructed. The three SentenceTransformer-based fast models (MiniLM, GTE-large, BGE-large, BGE-M3, Nomic) are comparatively quick to encode at this scale; the four large, AutoModel-based encoders (Linq-Embed-Mistral, E5-Mistral, SFR-Mistral, all 7B parameters fp16, plus ColBERTv2's per-token encoding) are markedly slower, the original 98\,716-company Linq-Embed-Mistral run took approximately 34 minutes of GPU time, spread across several resubmitted jobs due to this cluster's per-job wall-time limit and lengthy model-loading time. At roughly 4$\times$ the scale, each of these slower encoders is expected to take on the order of 2-2.5 hours of cumulative GPU time. The resulting embeddings, together with per-tier BM25 indexes and an evaluation notebook, are intended to answer two separate questions: (1) does Recall@$k$/NDCG@$k$ and query latency hold up as the corpus grows from roughly 100K to 400K companies, using the queries with the deepest and most reliable gold-standard coverage (the original 5-query pilot and the headline queries from Section~\ref{subsec:active_learning}) so that the resulting metrics are not read from an unreliably shallow label set; and (2) does the original baseline-model ranking (Section on baseline comparisons) survive re-evaluation against the non-circular gold standard. These results will be reported here once available.
